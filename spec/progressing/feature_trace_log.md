...
### Common fields for any scenario:
...
- `_err`: The error flag, true if there is an error. Optional, default is false. Type=boolean
...
### RPC
While service A(@192.168.0.101:8080) calls service B(@192.168.0.102:8081), both of them prints respective logs.
RPC logs should include the following additional fields:
- `_duration`: The duration of the RPC call in milliseconds. Optional, can be added by logging system if not provided. Type=int
...
- `_duration`: The duration of the redis call in milliseconds. Optional, can be added by logging system if not provided. Type=int
...
- `_duration`: The duration of the SQL execution in milliseconds. Optional, can be added by logging system if not provided. Type=int
- `_sql`: The actual SQL statement being executed. Optional. Truncated if there are more than 1024 UTF-8 characters.
- `_dur`: The duration of the SQL execution in milliseconds. Optional, can be added by logging system if not provided.
